  ...
  handler: async (ctx, args): Promise<ApiResult<CuriaeDeepResponse>> => {
    ...
    if (!apiKey) {
      return fail<CuriaeDeepResponse>("Missing GEMINI_API_KEY.", {
        ...
        lawyerNeeded: "",
        jurisdictionNote: "",
        disclaimer: "This is not legal advice. Curiae is an AI tool. Curiae provides legal information, not legal representation. Consult a qualified lawyer for serious matters.",
        urgent: false,
        raw: ""
      });
    }
...
export const analyzeDocument = action({
  ...
  handler: async (ctx, args): Promise<ApiResult<DocumentAnalysisResponse>> => {
    ...
    await ctx.runMutation(api.documents.updateDocumentAnalysis, {
